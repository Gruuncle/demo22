$(function () {
					...
					$(".del_sure", Wparent).click(function () {
						...
						$.ajax({
							...
							success: function (msg) {
								...
								var delSuccess = "<p class='del_info'>" + res.msg + "</p><div class='tip_btnbox'><button class='btn btn_dsure del_close'>确认</button></div>";
								$("body", Wparent).append(tipBg);
								$("body", Wparent).append(tipBox);
				...
				$("body").on("click", ".up_shelf", function () {
					...
					$(".del_sure", Wparent).click(function () {
						...
						$.ajax({
							type: "post",
							url: obj.shelfUrl,
							data: "id=" + dataId + "&&goods_state=0", //0为需要下架，1为需要上架
							success: function (msg) {
								Tipclose();
				...
				$("body").on("click", ".down_shelf", function () {
					...
					$(".del_sure", Wparent).click(function () {
						...
						$.ajax({
							type: "post",
							url: obj.shelfUrl,
							data: "id=" + dataId + "&&shegoods_state=1", //0为需要下架，1为需要上架
							success: function (msg) {
								Tipclose();
	...
	$.fn.other = function (options) {
		...
		return this.each(function () {
			...
			$(".submit_user").click(function () {
				// debugger
				// console.log($(this).parent().children().parent().parent().children()[0].getAttribute("id"));
				// debugger
				...
				if (validform().form()) {
					...
					// console.log('4');
					// console.log(opts.formUrl);
					var json = $(opts.formId).serialize();
					var data_list = json.split('fullname')[0];
					var data2 = data_list.split('&');
					username = data2[0].split('=')[1];
					phone = data2[1].split('=')[1];

					var a = $(this).parent().children().parent().parent().children().children()[1].value;
					var n = $(this).parent().children().parent().parent().children().children()[3].value
					$.ajax({
						type: "post",
						url: '/api/user/updata_user',
						data: {
							'username': $(this).parent().children().parent().parent().children().children()[1].value,
							'phone': phone,
							'id': $(this).parent().children().parent().parent().children()[0].getAttribute("id")
						},
						success: function (msg) {
							debugger
							var res = JSON.parse(msg);
			...
			$(".submit_addcar").click(function () {
				var id = $(this).attr('id');
				var link = opts.linkHref,
					that = $(this);
				if (validform().form()) {
					that.prop("disabled", "disabled");
					console.log($(opts.formId).serialize());
					var new_s = $(opts.formId).serialize() + '&user_id=' + id + ''
					$.ajax({
						type: "post",
						url: opts.formUrl,
						data: new_s,
			...
			$(".submit_addyearcar").click(function () {
				//获取当前时间，格式YYYY-MM-DD
			...
			function getEndFormatDate() {
				...
				if (validform().form()) {
					that.prop("disabled", "disabled");
					console.log($(opts.formId).serialize());
					var new_s = $(opts.formId).serialize() + '&user_id=' + id + '&start_time='+getNowFormatDate()+'&end_time='+getEndFormatDate()+''
					$.ajax({
						type: "post",
						url: opts.formUrl,
						data: new_s,
			...
			$(".submit_updatacar").click(function () {
				var id = $(this).attr('id');
				var link = opts.linkHref,
					that = $(this);
				if (validform().form()) {
					that.prop("disabled", "disabled");
					console.log($(opts.formId).serialize());
					var new_s = $(opts.formId).serialize() + '&id=' + id + ''
					$.ajax({
						type: "post",
						url: opts.formUrl,
						data: new_s,
	...
	jQuery.selectSearch = function (elm) {
		...
		elm.find(".select_search").css({
			"width": (boxW - 24) + "px"
		});
		elm.find(".ss_cont").css({
			"width": (boxW - 46) + "px"
		});
		//显示下拉
